...
RSpec.describe Notification, type: :model do
  ...
  context '有効' do
    it '必要な値がある時有効' do
      expect { FactoryBot.create(:notification) }.to change(described_class, :count).by(1)
    end

    it 'post_idがある時有効' do
      expect { FactoryBot.create(:notification, post_id: 1) }.to change(described_class, :count).by(1)
    end

    it 'comment_idがある時有効' do
      expect { FactoryBot.create(:notification, comment_id: 1) }.to change(described_class, :count).by(1)
    end
  end
end
